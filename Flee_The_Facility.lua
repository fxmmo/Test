...
local function clearAllHighlights(highlightName)
    ...
    if CurrentMap and CurrentMap then
        for _, obj in pairs(CurrentMap:GetChildren()) do
...
local function triggerHelp()
    ...
    if not char or not CurrentMap then return end
...
task.spawn(function()
    ...
    while running do
        ...
        if currentTime - lastESPScan >= 1.5 then
            ...
            if (tableHighlightEnabled or podHighlightEnabled) and CurrentMap and CurrentMap then
                for _, obj in pairs(CurrentMap:GetChildren()) do
